...
const bot_manager = async function() {
    ...
    // Create initial AI service (will be updated with game instance if using winning strategy)
    let ai_service = ai_service_factory.createAIService(ai_config.provider, ai_config.model_name, ai_config.playstyle);
    ...
    // If using winning strategy, update the AI service with game instance after game is initialized
    if (ai_config.playstyle === "winning") {
        bot.setAIServiceFactory(ai_service_factory);
    }
